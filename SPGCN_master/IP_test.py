...
cps=sys.argv[2]
# iter=str(0)
test_data=np.load('pretreatment_val/IP_test_data.npy')
test_label=np.load('pretreatment_val/IP_test_label.npy')
test_location=np.load('pretreatment_val/IP_test_location('+iter+').npy')

data = loadmat('data/Indian_pines_corrected.mat')
label = loadmat('data/Indian_pines_gt.mat')
...
test_input_data=select_local(test_data,test_location)
...
# dist = tf.exp(a / S)
# dist = dist + tf.eye(crop_size[0] * crop_size[0])
# dist = dist / tf.reduce_sum(dist, 2, keepdims=True)
...
with tf.Session(config=config) as sess:
    ...
    saver.restore(sess, "wjy_data/IP_model.ckpt")
    # if (n==10):
    #     n=0
    #     m=m+1
    # # if epoch > 0:
    ...
    for i in tqdm(range(math.ceil(test_data.shape[0] / cal_batch))):
        if start + cal_batch <= test_data.shape[0]:
            feed_dict = {data_placeholder: test_input_data[start:start + cal_batch],
                         label_placeholder: test_label[start:start + cal_batch], tf_is_training: False}
            ...
            lab = np.float32(np.argmax(test_label[start:start + cal_batch], 1))
            ...
            feed_dict = {data_placeholder: test_input_data[start:test_data.shape[0]],
                         label_placeholder: test_label[start:test_data.shape[0]], tf_is_training: False}
            # for j in range(len(adj_placeholder)):
            #     feed_dict.update({adj_placeholder[j]: test_adj_list_whole[i][j]})
            # feed_dict.update(
            #     {dist_placeholder: np.eye(crop_size[0] ** 2)[np.newaxis,:]})
            ...
            lab = np.float32(np.argmax(test_label[start:test_data.shape[0]], 1))
            lab = np.append(lab, -2)
            for k in range(classes):
                index = (lab == k)
                f = np.sum((predict[index] == lab[index]) != 0)
...
test_time = (toc - tic).total_seconds()
print('test_time:', test_time)
with open("./IP_time_result.txt", "a") as f:
    ...
    np.save('pretreatment_val/IP_test_result(' + iter + ').npy', all_class)
    f.close()
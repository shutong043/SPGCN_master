...
cps=sys.argv[2]
# iter=str(21)
train_data=np.load('pretreatment_val/IP_train_data.npy')
train_label=np.load('pretreatment_val/IP_train_label.npy')
train_location=np.load('pretreatment_val/IP_train_location.npy')

val_data=np.load('pretreatment_val/IP_val_data.npy')
val_label=np.load('pretreatment_val/IP_val_label.npy')
val_location=np.load('pretreatment_val/IP_val_location('+iter+').npy')
...
#         if (epoch>=800 and epoch%20==0) or (accur==1):
        ...
        #     break
            ...
            if AA > (val_AA-0.003) and accur > (val_OA-0.003) and ce_los_train<15:
                block=0
                print('------------------------------------------------------------------save-------------------------------------------------------------------')
                iters = epoch
                saver_path = saver.save(sess,"wjy_data/IP_model.ckpt")
                cum = cum + 1
                if cum == 10:
                    break
                if AA > val_AA:

...
cps=sys.argv[2]
# iter=str(21)
test_data=np.load('pretreatment_val/Salinas_test_data.npy')
test_label=np.load('pretreatment_val/Salinas_test_label.npy')
test_location=np.load('pretreatment_val/Salinas_test_location('+iter+').npy')

data = loadmat('data/Salinas_corrected.mat')
label = loadmat('data/Salinas_gt.mat')
data=data['salinas_corrected']
label=label['salinas_gt']
...
#         tile_data = tf.expand_dims(data_placeholder[:,i*gen_batch:(i+1)*gen_batch,:half_spectral], axis=-2)
#         minus = tf.subtract(tile_data, tf.expand_dims(data_placeholder[:,:,:half_spectral], axis=1))
#         a1 = -tf.reduce_sum(tf.square(minus), -1)
#         tile_data = tf.expand_dims(data_placeholder[:, i * gen_batch:(i + 1) * gen_batch, half_spectral:], axis=-2)
#         minus = tf.subtract(tile_data, tf.expand_dims(data_placeholder[:,:,half_spectral:], axis=1))
#         a2 = -tf.reduce_sum(tf.square(minus), -1)
#         a=a1+a2
#         if i==0:
#             dist=a
#         else:
...
#     else:
#         tile_data = tf.expand_dims(data_placeholder[:,i*gen_batch:,:half_spectral], axis=-2)
#         minus = tf.subtract(tile_data, tf.expand_dims(data_placeholder[:,:,:half_spectral], axis=1))
#         a1 = -tf.reduce_sum(tf.square(minus), -1)
#         tile_data = tf.expand_dims(data_placeholder[:, i * gen_batch:, half_spectral:], axis=-2)
#         minus = tf.subtract(tile_data, tf.expand_dims(data_placeholder[:,:,half_spectral:], axis=1))
...
for i in tqdm(range(gen_batch_size)):
    if i != gen_batch_size-1:
        ...
        # minus = tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:(i+1)*gen_batch,], axis=-2), tf.expand_dims(data_placeholder, axis=1))
        # a = -tf.reduce_sum(tf.square(tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:(i+1)*gen_batch,], axis=-2), tf.expand_dims(data_placeholder, axis=1))), -1)
        if i==0:
            dist=-tf.reduce_sum(tf.square(tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:(i+1)*gen_batch,], axis=-2), tf.expand_dims(data_placeholder, axis=1))), -1)
        else:
            dist=tf.concat([dist, -tf.reduce_sum(tf.square(tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:(i+1)*gen_batch,], axis=-2), tf.expand_dims(data_placeholder, axis=1))), -1)], 1)
    else:
        # tile_data = tf.expand_dims(data_placeholder[:,i*gen_batch:,], axis=-2)
        # minus = tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:,], axis=-2), tf.expand_dims(data_placeholder, axis=1))
        # a = -tf.reduce_sum(tf.square(tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:,], axis=-2), tf.expand_dims(data_placeholder, axis=1))), -1)
        dist = tf.concat([dist, -tf.reduce_sum(tf.square(tf.subtract(tf.expand_dims(data_placeholder[:,i*gen_batch:,], axis=-2), tf.expand_dims(data_placeholder, axis=1))), -1)], 1)
dist = tf.exp(dist / S)
...
with tf.Session(config=config) as sess:
    saver.restore(sess, "wjy_data/Salinas_model"+iter+".ckpt")
    for i in range(big_batch_num):
        if i != big_batch_num - 1:
            test_input_data = select_local(test_data[i * big_batch:(i + 1) * big_batch],
                                           test_location[i * big_batch:(i + 1) * big_batch])
...
with open("./Salinas_time_result.txt", "a") as f:
    ...
    np.save('pretreatment_val/Salinas_test_result(' + iter + ').npy', all_class)
    f.close()